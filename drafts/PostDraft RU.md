...
DSL KeyPad — программа с обширным арсеналом символов, доступных прямо через клавиатуру. Пользователю больше не нужно иметь множество отдельных языковых раскладок — буквы самых разных языков доступны с русской и английской ракладок. Это не ограничено латиницей и кириллицей — можно писать даже [Германскими рунами](https://ru.wikipedia.org/wiki/Руны) или [Глаголицей](https://ru.wikipedia.org/wiki/Глаголица).

Имеются и широкие возможности к кастомизации инструмента под свои нужды.

У меня самой был запрос на подобный инструмент — существующие решения недостаточно мне подходили и я создала своё, под себя и для себя. Надеюсь, что оно окажется полезным и для других людей.

А это примеры названий, написанных с помощью DSL KeyPad:
...
Эта программа даёт пользователю доступ к внушительному перечню символов Юникода прямо с клавиатуры. Основной фокус — увеличение охвата латиницы и кириллицы без надобности в отдельных языковых раскладках, помимо английской и русской. Но этим инструмент не ограничивается.

Первый нюанс — программа плохо совместима с быстрой печатью. Если вам необходима высокая скорость ввода символов — это не тот инструмент, лучше использовать раскладки для отдельных языков.
...
«DSL KeyPad» родился из собственной нужды к использованию «нестандартных» символов. Сначала я пользовалась «[Типографской раскладкой](https://ilyabirman.ru/typography-layout/)» Ильи Бирмана (и пользуюсь по сей день в паре со своим инструментом), но этого мне оказалось мало — она не давала мне всего необходимого.

Я искала и пробовала другие решения — например, [WinCompose](https://github.com/samhocevar/wincompose), — но по итогу это привело меня к мысли «всё не совсем то». Тогда я уже занималась написанием своего скрипта, ещё совсем простого, и решила продолжить его развивать.
...
Работает через строки автозамены, требуя вводить текст формата `\tcalc{<ИЗ><В>}`. После срабатывания активируется ввод значения для конвертации, а нажатием любой клавиши, не являющейся цифрой, запятой, точкой или «'» подтверждается вывод конвертированного значения в поле ввода.
...
PS. Благодарю дочитавших. Надеюсь, что мой инструмент кого-то заинтересует и окажется полезным не только для меня.
