...
Альт-коды можно вводить в шестнадцатиричном формате: `A+0B9` = `A+0185` → № (Windows-1251). Для этого ввод должен содержать букву из диапазона A–F. Верхняя граница — A+FF (A+255).
...
Строки автозамены формата `$><LaTeX-команда>$`. Если в библиотеке зарегистрирована какая-либо LaTeX-команда, то закреплённый за ним символ можно ввести и таким способом, примеры: `$>\AE$` → Æ, `$>\={\AE}$` → Ǣ, `$>\Omega$` → Ω, `$>\textpertenthousand$` → ‱.
