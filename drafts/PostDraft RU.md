...
1. Потребляет ~140–180 МБ ОЗУ. По возможности буду пытаться оптимизировать использование памяти. Так, например, раньше вызов главной панели со списками символов потреблял +20 МБ ОЗУ на каждое открытие, сейчас же +5 МБ (что тоже, как по мне, многовато для *каждого* открытия, но лучше, чем +20).
