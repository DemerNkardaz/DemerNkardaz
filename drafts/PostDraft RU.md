...
Доступны следующие шкалы: Цельсий (C), Фаренгейт (F), Кельвин (K), Делиль (D), Лейден (L), Ньютон (N), Уэджвуд (W), Ранкин (RA), Реомюр (RE), Рёмер (RO).

Ещё доступны вараинты формата вывода, выбираемые при указании дробной части (саму дробную часть можно опустить, например — «450,»):
...
Строки автозамены формата `$<LaTeX-код>$`. Если в библиотеке зарегистрирован какой-либо LaTeX код, то закреплённый за ним символ можно ввести и таким способом, примеры: `$\AE$` → Æ, `$\={\AE}$` → Ǣ, `$\Omega$` → Ω, `$\textpertenthousand$` → ‱.
