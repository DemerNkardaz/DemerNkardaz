...
В директории «`DSLKeyPad\Mods\`» можно разрешать сторонние AHK-файлы в качестве «модов» к программе, однако они должны содержать в начале имени `pre_init__` (для загрузки до инициализации) или `post_init__` (для загрузки после инициализации).

Файлы модов автоматически подхватываются. После добавления нового файла следует подождать 5 секунд перед перезапуском программы.

Для отключения мода, помимо удаления его файла, можно найти его имя в `DSLKeyPad\Mods\_modlist.ini` и установить занчение `0`.
